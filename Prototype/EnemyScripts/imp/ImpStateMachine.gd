...
class_name ImpStateMachine

export(Array, ImpAnim.Type) var cancelFilter = []
#var emptyFilter = []
...
func transition(nextStateName : String, cancel : bool = false) -> bool:
	...
	if cancel:
		var cancelStateIndex = cancelFilter.find(ImpAnim.strToEnum(nextStateName))
